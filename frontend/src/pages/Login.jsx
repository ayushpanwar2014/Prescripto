      ...
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] border border-zinc-200 sm:min-w-96 rounded-xl text-zinc-600 text-sm shadow-2xl'>
        ...
        <p>Please {state === 'Sign Up' ? 'sign up' : 'log in'} to book appointments.</p>
        {
          ...
          <div className='w-full'>
            <p>Full Name</p>
            <input className='border border-zinc-300 rounded w-full p-2 mt-1' type="text" onChange={onChangeHandler} value={signUp.name} name='name' required />
          </div>
        }
        <div className='w-full'>
          <p>Email</p>
          <input className='border border-zinc-300 rounded w-full p-2 mt-1' type="email" onChange={onChangeHandler} value={signUp.email} name='email' required />
        </div>
        <div className='w-full'>
          <p>Password</p>
          <input className='border border-zinc-300 rounded w-full p-2 mt-1' type="password" onChange={onChangeHandler} value={signUp.password} name='password' required />
        </div>
